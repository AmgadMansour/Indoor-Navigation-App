...
public class FileAccess {
	...
	public String save(
		...
		for (DefaultVertex v : graph.vertexSet()) {
			...
			vertexJson.put("x", v.getCoordinate().getX());
			vertexJson.put("y", v.getCoordinate().getY());
	...
	public void load(
		...
		Iterator i = vertices.keys(); i.hasNext();) {
			...
			DefaultVertex vertex = new DefaultVertex(new Coordinate(
					getTrueCoord((float) vertexJson.getDouble("x")), getTrueCoord((float) vertexJson.getDouble("y"))));
			vertex.setLabel(key);
	...
	public float getTrueCoord(float c) {
		DisplayMetrics metrics = Resources.getSystem().getDisplayMetrics();


		if (metrics.xdpi >= 480.0f) {
			return (c * metrics.xdpi) / 480.0f;
		}
		else {
			return (c * 480.0f) / metrics.xdpi;
		}
